/*
 * Memory manager/heap.c
 * Ported by The Neuromancer <neuromancer at paranoici dot org>
 * from Doug Lea's dlmalloc (ftp://gee.cs.oswego.edu/pub/misc/malloc.c)
 *
...
static void *heap_end = (void *)0x00800000;

static void *mm_heap_adjust(ssize_t delta)
{
void *ret = heap_end;

	delta = size_in_pages(delta);

	if (delta > 0) {
		if (mm_map((uint32_t)heap_end >> 12, delta, CPU_PAGE_FLAG_WRITABLE))
			return (void *)0;
		...
		mm_unmap((uint32_t)heap_end >> 12 - delta, delta);
...
#define MORECORE mm_heap_adjust
...
#define MORECORE_FAILURE ((void *)0)
...
#define malloc_getpagesize (CPU_PAGE_SIZE) 
...
static void*  private_mm_heap_allocate(size_t);
static void   private_mm_heap_free(void*);
static void*  private_mm_heap_reallocate(void*, size_t);
static void*  private_mm_heap_allocate_aligned(size_t, size_t);
...
void* mm_heap_allocate(size_t bytes) {
  ...
  m = private_mm_heap_allocate(bytes);
  if (MALLOC_POSTACTION != 0) {
  }
  return m;
}

void mm_heap_free(void* m) {
  if (MALLOC_PREACTION != 0) {
    return;
  }
  private_mm_heap_free(m);
  if (MALLOC_POSTACTION != 0) {
  }
}

void* mm_heap_reallocate(void* m, size_t bytes) {
  if (MALLOC_PREACTION != 0) {
    return 0;
  }
  m = private_mm_heap_reallocate(m, bytes);
  if (MALLOC_POSTACTION != 0) {
  }
  return m;
}
...
void* mm_heap_allocate_aligned(size_t alignment, size_t bytes) {
  ...
  m = private_mm_heap_allocate_aligned(alignment, bytes);
  if (MALLOC_POSTACTION != 0) {
  }
  return m;
}
...
#define MALLOC_COPY(dest, src, nbytes)  memory_copy(dest, src, nbytes)
#define MALLOC_ZERO(dest, nbytes)       memory_clear(dest, nbytes)
...
static void* sysmalloc(INTERNAL_SIZE_T nb, mstate av)
{
  ...
  */
  ...
  if (have_fastchunks(av)) {
    ...
    return private_mm_heap_allocate(nb - MALLOC_ALIGN_MASK);
  }
        ...
        if (old_size != 0) {
          ...
          */
          if (old_size >= MINSIZE) {
            ...
            private_mm_heap_free(chunk2mem(old_top));
...
void* private_mm_heap_allocate(size_t bytes)
{
...
void private_mm_heap_free(void* mem)
{
  mstate av = get_malloc_state();
...
void* private_mm_heap_reallocate(void* oldmem, size_t bytes)
{
  mstate av = get_malloc_state();
  ...
  if (bytes == 0) {
    private_mm_heap_free(oldmem);
    return 0;
  }
#endif

  /* realloc of null is supposed to be same as malloc */
  if (oldmem == 0) return private_mm_heap_allocate(bytes);
  ...
  if (!chunk_is_mmapped(oldp)) {
    ...
    if ((CHUNK_SIZE_T)(oldsize) >= (CHUNK_SIZE_T)(nb)) {
      ...
      /* allocate, copy, free */
        ...
        newmem = private_mm_heap_allocate(nb - MALLOC_ALIGN_MASK);
        if (newmem == 0)
    ...
    if (remainder_size < MINSIZE) { /* not enough extra to split off */
      ...
      private_mm_heap_free(chunk2mem(remainder)); 
    }
...
void* private_mm_heap_allocate_aligned(size_t alignment, size_t bytes)
{
  INTERNAL_SIZE_T nb;             /* padded  request size */
  ...
  if (alignment <= MALLOC_ALIGNMENT) return private_mm_heap_allocate(bytes);
  ...
  */
  ...
  m  = (char*)(private_mm_heap_allocate(nb + alignment + MINSIZE));
  ...
  if ((((PTR_UINT)(m)) % alignment) != 0) { /* misaligned */
    ...
    */
    ...
    private_mm_heap_free(chunk2mem(p));
    p = newp;
    ...
    V2.6.6 Sun Dec  5 07:42:19 1999  Doug Lea  (dl at gee)
      ...
      * Added Several WIN32 cleanups from Martin C. Fong <mcfong at yahoo.com>
         ...
         * Improve WIN32 'sbrk()' emulation's 'findRegion()' routine to
           avoid infinite loop
      * Always call 'private_mm_heap_free()' rather than 'free()'
